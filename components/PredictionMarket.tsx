...
const TrendGraph: React.FC<{ yesProb: number; marketId: string; height?: number }> = ({ yesProb, marketId, height = 200 }) => {
  ...
  return (
    <div className="w-full relative flex bg-[#0d1117] rounded-xl border border-white/5 my-6 overflow-hidden shadow-2xl group/graph" style={{ height: `${height}px` }}>
      ...
      <div className="w-10 md:w-14 flex flex-col justify-between items-center py-0.5 opacity-40 text-[9px] md:text-[10px] font-black text-white bg-black/40">
        {labels.map((val) => (
...
const MerketDetailModal: React.FC<{ merket: MerketType; onClose: () => void; onVote: (id: string, option: 'YES' | 'NO') => void; isVoting: boolean }> = ({ merket, onClose, onVote, isVoting }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center bg-black/95 backdrop-blur-md md:p-4">
      {/* Mobile: Full Screen / Desktop: Modal */}
      <div className="bg-[#0f172a] w-full md:max-w-5xl h-[100dvh] md:h-[90vh] md:max-h-[800px] rounded-none md:rounded-[2.5rem] border-t md:border border-white/10 overflow-hidden flex flex-col md:flex-row shadow-2xl text-white relative">
        <button onClick={onClose} className="absolute top-4 right-4 md:top-6 md:right-6 z-20 p-2 bg-white/5 border border-white/10 rounded-full hover:scale-110 transition-transform"><X size={20} /></button>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <div className="p-5 md:p-12 overflow-y-auto custom-scroll flex-1">
            <div className="flex items-start md:items-center gap-4 md:gap-6 mb-6 md:mb-8 pr-10">
              <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl bg-blue-600 p-0.5 shrink-0 border border-white/20 shadow-[0_0_20px_rgba(37,99,235,0.3)]">
                <img src={merket.image || BRAND_LOGO} className="w-full h-full object-cover rounded-lg" />
              </div>
              <h2 className="text-2xl md:text-5xl font-black uppercase italic tracking-tighter leading-[0.9] break-words">{merket.question}</h2>
            </div>
            ...
            {mcap && (
              <div className="mb-6 flex items-center gap-3 bg-blue-500/10 border border-blue-500/20 px-4 py-2 md:px-6 md:py-3 rounded-2xl w-fit">
                ...
                <span className="text-blue-400 font-black uppercase italic tracking-widest text-xs md:text-sm">MCAP: ${mcap}</span>
              </div>
            )}
            ...
            <div className="mt-8 border-t border-white/10 pt-6 md:pt-8 pb-20 md:pb-0">
              <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/30 mb-4 md:mb-6 flex items-center gap-2 italic">
              ...
              <div ref={scrollRef} className="space-y-3 md:space-y-4 max-h-48 md:max-h-64 overflow-y-auto custom-scroll pr-2 md:pr-4 mb-4 md:mb-6">
                {comments.length === 0 ? <div className="text-center py-8 opacity-20 italic">Awaiting terminal signals...</div> :
                  comments.map((c) => (
                    <div key={c.id} className="bg-white/[0.03] rounded-2xl p-3 md:p-4 border border-white/5">
                      <div className="flex justify-between items-center mb-1"><span className="text-[9px] md:text-[10px] font-black text-blue-400 uppercase tracking-widest">{c.username}</span><span className="text-[8px] md:text-[9px] text-white/20 font-mono italic">{new Date(c.created_at).toLocaleTimeString()}</span></div>
                      <p className="text-xs md:text-sm font-bold text-white/70">{c.content}</p>
                    </div>
                  ))
                }
              </div>
              <form onSubmit={handlePostComment} className="flex gap-2 bg-black/40 p-2 rounded-2xl border border-white/10 focus-within:border-blue-500/50 transition-colors">
                <input required type="text" placeholder="Alias" className="w-20 md:w-24 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-[9px] md:text-[10px] font-black text-blue-400 placeholder-white/20 focus:outline-none uppercase" value={username} onChange={(e) => setUsername(e.target.value)} />
                <input required type="text" placeholder="Type message..." className="flex-1 bg-transparent border-none px-2 md:px-3 py-2 text-xs md:text-sm font-bold text-white placeholder-white/10 focus:outline-none" value={newComment} onChange={(e) => setNewComment(e.target.value)} />
        ...
        <div className="w-full md:w-80 bg-[#0f172a] md:bg-white/[0.01] p-5 md:p-12 flex flex-col justify-center gap-3 md:gap-4 border-t border-white/10 md:border-t-0 md:border-l z-10 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] md:shadow-none pb-8 md:pb-12">
          ...
          <div className="grid grid-cols-1 gap-3 mt-2 md:mt-6">
            <button onClick={handleTweetAction} className="py-3 md:py-4 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center gap-2 text-[9px] md:text-[10px] font-black uppercase italic hover:bg-white/10 transition-all text-white/60">
...
const MerketCard: React.FC<{ merket: MerketType; onOpen: (m: MerketType) => void }> = ({ merket, onOpen }) => {
  ...
  return (
    <div className="bg-[#1e293b] border border-white/5 rounded-3xl p-5 md:p-6 flex flex-col h-full hover:bg-[#253247] transition-all cursor-pointer group shadow-xl relative overflow-hidden" onClick={() => onOpen(merket)}>
      <div className="flex justify-between items-start gap-3 md:gap-4 mb-4 min-h-[56px] md:min-h-[64px]">
        <div className="flex-1">
          <h3 className="text-base md:text-xl font-black text-white leading-tight group-hover:text-blue-400 transition-colors line-clamp-2 uppercase italic tracking-tighter mb-2">{merket.question}</h3>
          {mcap && (
        ...
        <div className="shrink-0"><img src={merket.image || BRAND_LOGO} className="w-10 h-10 md:w-10 md:h-10 rounded-xl border border-white/10 object-cover shadow-lg" /></div>
      </div>
      ...
      <div className="mt-auto pt-3 md:pt-4 border-t border-white/5 flex justify-between items-center text-[9px] md:text-[10px] font-black uppercase italic text-white/20 tracking-[0.2em]">
        ...
        <div className="flex items-center gap-2"><Plus size={12} className="opacity-40 group-hover:opacity-100 transition-opacity md:w-[14px] md:h-[14px]" /></div>
      </div>
    </div>
  );
};
...
const CreateMarketModal: React.FC<CreateMarketModalProps> = ({ onClose, onCreated, initialQuestion = '', initialDescription = '' }) => {
    ...
    return (
        <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4">
            <div className="bg-white w-full max-w-xl rounded-3xl md:rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden shadow-2xl text-blue-900 max-h-[90vh] overflow-y-auto custom-scroll">
                <button onClick={onClose} className="absolute top-4 right-4 md:top-6 md:right-6 p-2 bg-blue-100 text-blue-900 rounded-full"><X size={18} className="md:w-[20px] md:h-[20px]" /></button>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-black py-4 md:py-5 rounded-2xl text-lg md:text-xl hover:bg-blue-700 transition-all disabled:opacity-50 uppercase italic tracking-tighter shadow-lg">{loading ? <Loader2 className="animate-spin mx-auto" /> : "Initiate Terminal"}</button>
                </form>
            </div>
        </div>
    );
  ...
  return (
    <section id="merkets">
      <div className="container mx-auto">
        <div className="flex flex-wrap items-center gap-2 mb-8 md:mb-12 pb-5 border-b border-white/5">
            <div className="flex items-center bg-white/5 p-1 rounded-2xl border border-white/10 shadow-lg overflow-x-auto max-w-[calc(100%-120px)] md:max-w-none no-scrollbar">
                <button onClick={() => setActiveFilter('top')} className={`px-4 md:px-5 py-2 md:py-2.5 rounded-xl font-black text-[10px] md:text-xs uppercase italic tracking-widest transition-all ${activeFilter === 'top' ? 'bg-blue-600 text-white shadow-xl' : 'text-white/40 hover:text-white'}`}>Top</button>
                <button onClick={() => setActiveFilter('new')} className={`px-4 md:px-5 py-2 md:py-2.5 rounded-xl font-black text-[10px] md:text-xs uppercase italic tracking-widest transition-all ${activeFilter === 'new' ? 'bg-blue-600 text-white shadow-xl' : 'text-white/40 hover:text-white'}`}>New</button>
                <button onClick={() => setActiveFilter('trending')} className={`px-4 md:px-5 py-2 md:py-2.5 rounded-xl font-black text-[10px] md:text-xs uppercase italic tracking-widest transition-all ${activeFilter === 'trending' ? 'bg-blue-600 text-white shadow-xl' : 'text-white/40 hover:text-white'}`}>Trending</button>
            </div>
            <div className="ml-auto">
                <button onClick={() => setIsCreateOpen(true)} className="flex items-center gap-2 px-4 md:px-6 py-2 md:py-3 bg-blue-500 text-white rounded-2xl font-black text-[10px] md:text-xs uppercase italic tracking-tighter hover:bg-blue-600 transition-all shadow-2xl shadow-blue-500/20 border border-blue-400/20">
