...
const BRAND_LOGO = "https://pbs.twimg.com/media/G8TVwV2XgAczA4R?format=jpg&name=large";
...
const MarketChart: React.FC<{ yesProb: number; marketId: string; optionA: string; optionB: string; }> = ({ yesProb, marketId, optionA, optionB }) => {
    ...
    return ( <div className="w-full h-[250px] bg-slate-950 rounded-xl border border-slate-800 p-2 my-6 shadow-2xl relative"> <canvas ref={canvasRef}></canvas> <div className="absolute bottom-2 left-3 bg-black/60 px-3 py-1 rounded-md text-[9px] font-bold text-slate-400 uppercase tracking-widest border border-slate-700 pointer-events-none"> SOURCE: COOLCAT ORACLE v1 </div> </div> );
};
    ...
    <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center bg-black/80 backdrop-blur-md md:p-4">
      <div className="bg-slate-900 w-full md:max-w-5xl h-[100dvh] md:h-[90vh] md:max-h-[800px] rounded-none md:rounded-3xl border-t md:border border-slate-700 overflow-hidden flex flex-col md:flex-row shadow-2xl text-white relative">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <div className="p-5 md:p-12 overflow-y-auto custom-scroll flex-1">
            <div className="flex items-start md:items-center gap-4 md:gap-6 mb-6 md:mb-8 pr-10">
              <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl bg-purple-600 p-0.5 shrink-0 border border-slate-700 shadow-[0_0_20px_rgba(168,85,247,0.3)]"><img src={merket.image || BRAND_LOGO} className="w-full h-full object-cover rounded-lg" /></div>
            ...
            {merket.marketType === 'MCAP_TARGET' && (
                <div className="mb-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700"><div className="text-[9px] uppercase font-bold text-slate-400 tracking-widest flex items-center gap-1.5"><Target size={10} /> Target MCAP</div><div className="text-lg font-bold text-purple-400">${targetMcapFormatted}</div></div>
                        <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700"><div className="text-[9px] uppercase font-bold text-slate-400 tracking-widest flex items-center gap-1.5"><Clock size={10} /> Time Left</div><div className="text-lg font-mono font-bold text-purple-400">{merket.expiresAt && <Countdown expiry={merket.expiresAt}/>}</div></div>
                    </div>
                    <div>
                        ...
                        <div className="h-2 bg-slate-700 rounded-full w-full"><div className="h-full bg-purple-500 rounded-full transition-all" style={{width: `${mcapProgress}%`}}></div></div>
                    </div>
                </div>
            )}
            ...
            <div className="mt-8 border-t border-slate-800 pt-6 md:pt-8 pb-20 md:pb-0">
              ...
              <form onSubmit={handlePostComment} className={`flex gap-2 bg-slate-950 p-2 rounded-2xl border border-slate-700 relative ${isResolved ? 'opacity-50' : 'focus-within:border-purple-500 transition-colors'}`}>
                ...
                <button type="submit" className="p-2 md:p-3 bg-purple-600 text-white rounded-xl hover:bg-purple-500 transition-colors shadow-lg" disabled={isResolved}><Send size={14} className="md:w-4 md:h-4" /></button>
                {!session && !isResolved && ( <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center rounded-2xl"> <button onClick={onAuthOpen} className="bg-purple-500 text-white font-bold py-2 px-4 rounded-lg">Sign In to Comment</button> </div> )}
              </form>
            </div>
          </div>
        </div>
        <div className="w-full md:w-80 bg-slate-950 p-5 md:p-12 flex flex-col justify-center gap-3 md:gap-4 border-t border-slate-700 md:border-t-0 md:border-l z-10 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] md:shadow-none pb-8 md:pb-12 relative">
            ...
            <button onClick={handleVoteClick} disabled={!selectedOption || isVoting || isResolved} className="w-full bg-purple-600 text-white font-bold py-4 md:py-5 rounded-2xl hover:bg-purple-500 transition-all disabled:opacity-30 disabled:cursor-not-allowed mt-2 md:mt-4 shadow-xl uppercase tracking-wider text-sm md:text-base">{isVoting ? <Loader2 className="animate-spin mx-auto" /> : (isResolved ? 'Market Closed' : 'Submit Vote')}</button>
            ...
            {!session && !isResolved && ( <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4"> <div className="text-center"> <h3 className="text-lg font-bold text-white mb-4">Login Required</h3> <p className="text-sm text-slate-400 mb-6">You must be logged in to cast a vote on this market.</p> <button onClick={onAuthOpen} className="w-full bg-purple-500 text-white font-bold py-3 px-4 rounded-lg">Sign In to Vote</button> </div> </div> )}
        </div>
      </div>
    </div>
  );
...
const MerketCard: React.FC<{ merket: MerketType; onOpen: (m: MerketType) => void }> = ({ merket, onOpen }) => {
  ...
  return (
    <div className={`bg-slate-800 border border-slate-700 rounded-3xl p-5 md:p-6 flex flex-col h-full hover:border-purple-500/50 transition-all group shadow-xl relative overflow-hidden ${isResolved ? 'opacity-60' : 'cursor-pointer'}`} onClick={() => !isResolved && onOpen(merket)}>
      ...
      <div className="flex justify-between items-start gap-3 md:gap-4 mb-4 min-h-[56px] md:min-h-[64px]">
        <div className="flex-1"> <h3 className="text-base md:text-lg font-bold text-slate-100 leading-tight group-hover:text-purple-400 transition-colors line-clamp-2 mb-2">{merket.question}</h3> {merket.marketType === 'MCAP_TARGET' && merket.targetMarketCap && ( <div className="flex items-center gap-1.5 text-purple-400 font-mono text-[9px] uppercase tracking-widest bg-purple-500/10 px-2 py-0.5 rounded w-fit border border-purple-500/20"> <Target size={10} /> MCAP TARGET: ${formatMcapTarget(merket.targetMarketCap)} </div> )} </div>
      ...
      <div className="mt-auto pt-4 border-t border-slate-700 flex justify-between items-center text-[10px] font-bold uppercase text-slate-500 tracking-wider">
        ...
        <div className="flex items-center gap-1 text-purple-500 opacity-0 group-hover:opacity-100 transition-opacity"><span>{isResolved ? 'Details' : 'Trade'}</span><Plus size={14} /></div>
      </div>
    </div>
  );
};
...
const CreateMarketModal: React.FC<{ onClose: () => void; onCreated: () => void; }> = ({ onClose, onCreated }) => {
    ...
    return ( <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4"> <div className="bg-slate-900 w-full max-w-xl rounded-3xl p-6 md:p-10 relative shadow-2xl text-white border border-slate-700 max-h-[90vh] overflow-y-auto custom-scroll"> <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-slate-800 text-slate-300 rounded-full border border-slate-700"><X size={20} /></button> <div className="mb-8"><h2 className="text-4xl font-bold tracking-tighter">New Market</h2><p className="text-purple-400 font-bold uppercase text-xs tracking-widest mt-1">Deploy terminal signal</p></div> <div className="flex bg-slate-800 p-1 rounded-full mb-6 border border-slate-700"><button onClick={()=>setMarketType('STANDARD')} className={`flex-1 py-2 rounded-full text-sm font-bold uppercase ${marketType==='STANDARD'?'bg-purple-600 text-white':'text-slate-400'}`}>Standard</button><button onClick={()=>setMarketType('MCAP_TARGET')} className={`flex-1 py-2 rounded-full text-sm font-bold uppercase ${marketType==='MCAP_TARGET'?'bg-purple-600 text-white':'text-slate-400'}`}>MCAP Target</button></div> <form onSubmit={handleSubmit} className="space-y-6"> {marketType === 'STANDARD' ? ( <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Question</label><input required type="text" placeholder="e.g., Will SOL flip ETH this cycle?" className="w-full bg-slate-800 border-2 border-slate-700 rounded-2xl p-4 font-medium text-slate-100 focus:outline-none focus:ring-4 focus:ring-purple-600/20 focus:border-purple-600" value={question} onChange={(e) => setQuestion(e.target.value)} /></div> ) : ( <> <div> <label className="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Solana Contract Address</label> <div className="relative"> <input required type="text" placeholder="Enter Token CA..." className="w-full bg-slate-800 border-2 border-slate-700 rounded-2xl p-4 font-mono text-slate-100 focus:outline-none focus:ring-4 focus:ring-purple-600/20 focus:border-purple-600 pr-20" value={contractAddress} onChange={(e) => setContractAddress(e.target.value)} /> {ticker && <span className="absolute right-4 top-1/2 -translate-y-1/2 bg-purple-500/20 text-purple-300 text-xs font-bold px-2 py-0.5 rounded-md border border-purple-500/30">${ticker}</span>} </div> </div> <div className="grid grid-cols-2 gap-4"> <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Target MCAP</label><input required type="text" placeholder="e.g., 15M, 250K" className="w-full bg-slate-800 border-2 border-slate-700 rounded-2xl p-4 font-mono text-slate-100 focus:outline-none focus:ring-4 focus:ring-purple-600/20 focus:border-purple-600" value={targetMcapStr} onChange={(e) => setTargetMcapStr(e.target.value)} /></div> <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Timeframe</label><select value={timeframe} onChange={(e)=>setTimeframe(Number(e.target.value))} className="w-full bg-slate-800 border-2 border-slate-700 rounded-2xl p-4 font-mono text-slate-100 focus:outline-none focus:ring-4 focus:ring-purple-600/20 focus:border-purple-600"><option value={24}>24 Hours</option><option value={72}>3 Days</option><option value={168}>7 Days</option></select></div> </div> </> )} <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Context (Optional)</label><textarea rows={2} placeholder="Provide details..." className="w-full bg-slate-800 border-2 border-slate-700 rounded-2xl p-4 font-medium text-slate-300 focus:outline-none focus:ring-4 focus:ring-purple-600/20 focus:border-purple-600 resize-none" value={description} onChange={(e) => setDescription(e.target.value)} /></div> <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2 ml-1 tracking-widest">Image</label><div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-700 rounded-2xl p-6 flex flex-col items-center justify-center bg-slate-800/50 hover:bg-slate-800/80 transition-all cursor-pointer relative min-h-[120px]">{preview ? <img src={preview} className="absolute inset-0 w-full h-full object-cover rounded-2xl opacity-80" /> : <Plus size={32} className="text-slate-600" />}<input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileChange} /></div></div> <button type="submit" disabled={loading} className="w-full bg-purple-600 text-white font-bold py-5 rounded-2xl text-xl hover:bg-purple-700 transition-all disabled:opacity-50 uppercase tracking-wider shadow-lg shadow-purple-500/20">{loading ? <Loader2 className="animate-spin mx-auto" /> : "Initiate Terminal"}</button> </form> </div> </div> );
};
      ...
      <div className="container mx-auto">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 md:mb-12 pb-5 border-b border-slate-800">
            <div className="flex items-center bg-slate-950 p-1 rounded-2xl border border-slate-800 shadow-lg w-full md:w-auto">
                <button onClick={() => setActiveCategory('general')} className={`flex-1 md:flex-none px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeCategory === 'general' ? 'bg-purple-600 text-white shadow-xl' : 'text-slate-400 hover:text-white'}`}>General Markets</button>
                <button onClick={() => setActiveCategory('mcap')} className={`flex-1 md:flex-none px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeCategory === 'mcap' ? 'bg-purple-600 text-white shadow-xl' : 'text-slate-400 hover:text-white'}`}>MCAP Targets</button>
            </div>
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
                ...
                <button onClick={handleCreateMarketOpen} className="flex items-center gap-2 px-4 py-2.5 bg-purple-500 text-white rounded-2xl font-bold text-xs uppercase tracking-wider hover:bg-purple-600 transition-all shadow-2xl shadow-purple-500/20 border border-purple-400/20 shrink-0">
                ...
                {session ? (
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={handleWalletLogin} disabled={walletLoading} className="px-4 py-2.5 bg-[#512da8] text-white rounded-2xl font-bold text-xs uppercase tracking-wider hover:bg-[#4527a0] transition-all border border-slate-700 shadow-lg disabled:opacity-50">
        ...
        {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4"><Loader2 className="animate-spin text-purple-500" size={56} /><span className="font-bold text-[10px] text-slate-500 uppercase tracking-widest">Syncing Terminal...</span></div>
        ) : (
